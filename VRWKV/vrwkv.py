...
wkv_cuda = load(name="wkv", sources=["/home/zixuwang/MyProjs/Vision-RWKV/segmentation/mmseg_custom/models/backbones/base/cuda/wkv_op.cpp", "/home/zixuwang/MyProjs/Vision-RWKV/segmentation/mmseg_custom/models/backbones/base/cuda/wkv_cuda.cu"],
...
class WKV(torch.autograd.Function):
    ...
    def forward(ctx, B, T, C, w, u, k, v):
        ...
        if half_mode:
            y = y.half()
        return y # [batch_size, total_patch_num, embed_dim]
...
class VRWKV_ChannelMix(BaseModule):
    ...
    def forward(self, x, patch_resolution):
        ...
        k = self.key(xk)
        k = torch.square(torch.relu(k)) # squared relu函数
        if self.key_norm is not None:
            k = self.key_norm(k)
        kv = self.value(k)
